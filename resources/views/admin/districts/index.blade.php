                                ...
                                <div class="col-sm-12">
                                    <table id="data-table" class="table table-striped table-bordered ">
                                        ...
                                        <tr client="row">
                                            ...
                                            <th>@sortablelink('name', 'Районы')</th>
                                        ...
                                        @foreach($districts as $district)
                                            <tr>
                                                <td>{{($districts->currentpage()-1)*$districts->perpage() +($loop->index+1)}}</td>
                                                <td>{{$district->name}}</td>
                                                <td>{{$district->region->name}}</td>
                                                <td>
{{--                                                    <a href="#modal-dialog-show" class="btn btn-xs btn-success"--}}
{{--                                                       onclick="showclient({{$region->id}})" title="Показать">--}}
{{--                                                        <i class="fa  fa-eye"></i>--}}
{{--                                                    </a>--}}
                                                    <a href="#modal-dialog-edit" class=" btn btn-xs btn-info"
                                                       title="Изменить">
                                                        <i class="fa fa-pencil-square-o"
                                                           onclick="getDistrictData({{$district->id}})"></i>
                                                    </a>
                                                    <a href="#modal-dialog-delete{{$district->id}}"
