...
<div id="content" class="content">
	...
	<ol class="breadcrumb pull-left">
		...
		<li class="active">Типы блогов</li>
	</ol>
    <div class="row">
                            ...
                            <div class="col-sm-12">
                                <table id="data-table" class="table table-striped table-bordered ">
                                    ...
                                    @foreach($blog_types as $blog_type)
                                        <tr>
                                            <td>{{($blog_types->currentpage()-1)*$blog_types->perpage() +($loop->index+1)}}</td>
                                            <td>{{$blog_type->name}}</td>
                                            <td>{{$blog_type->key}}</td>
                                            <td>
                                                <a href="#modal-dialog-show" class="btn btn-xs btn-success" onclick="showblog_type({{$blog_type->id}})" title="Показать">
                                                    <i class="fa  fa-eye"></i>
                                                </a>
                                                <a href="#modal-dialog-edit" class=" btn btn-xs btn-info" title="Изменить">
                                                    <i class="fa fa-pencil-square-o" onclick="getblog_typeData({{$blog_type->id}})"></i>
                                                </a>
                                                <a href="#modal-dialog-delete{{$blog_type->id}}"  class="btn btn-xs btn-danger"
                                                data-toggle="modal" title="Удалить">
                                                <i class="fa  fa-trash-o"></i>
                                                </a>
                                            </td>
                                        </tr>
                                        @include('admin.blog-types.delete')
                                    @endforeach
                                    </tbody>
                                </table>
                            </div>
...
@include('admin.blog-types.create')
@include('admin.blog-types.edit')
<div class="modal" id="modal-dialog-show">
    <div class="modal-dialog">
    </div>
</div>
@include('admin.blog-types._scripts')
</div>
@endsection
